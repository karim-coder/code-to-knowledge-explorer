...
import type { ParsedFunction, ParsedClass, Relationship, CodeMetrics } from "../lib/api";
...
export default function KnowledgeGraph({
  functions,
  classes,
  relationships = { function_calls: [], class_inheritance: [], method_calls: [], attribute_access: [] },
  metrics
}: KnowledgeGraphProps) {
  ...
  const transformDataToGraph = useCallback((): {
    ...
    relationships.function_calls.forEach((call, index) => {
      if (call.callee && nodeMap.has(call.callee)) {
        const callerId = call.caller && nodeMap.has(call.caller) ? nodeMap.get(call.caller) : null;
      ...
      style: [
        {
          ...
          style: {
            ...
            "font-size": (ele: cytoscape.NodeSingular) => {
              ...
              if (type === "class" && nodeData.methods && nodeData.methods.length > 5) {
                return "14px";
              }
              return "12px";
            },
            ...
            width: (ele: cytoscape.NodeSingular) => {
              ...
              if (type === "class") {
                baseWidth = 60 + (nodeData.methods ? nodeData.methods.length * 3 : 0);
            ...
            height: (ele: cytoscape.NodeSingular) => {
              ...
              if (type === "class") {
                baseHeight = 45 + (nodeData.methods ? nodeData.methods.length * 2 : 0);
              }
  ...
  }, [functions, classes, layout, transformDataToGraph]);
            ...
            <div>
              ...
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {selectedNode.methods.map((method, idx) => (
                  <div key={idx} className="text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 p-1 rounded">
